# Canais de referência — Dota 2

Lista de canais usados como referência de estilo e criatividade para gerar
ideias de vídeo. Use estes canais para entender formatos, tom e ângulos de
conteúdo já validados pela comunidade — não para copiar, mas para inspirar
variações e cruzamentos (ex.: "um replay breakdown no estilo do DotaCinema
sobre a jogada X do patch atual").

## 🎬 Highlights & Entretenimento
- DotaCinema
- Dota Watafak
- hOlyhexOr
- Jenkins Clips

## 📚 Guias & Educacional
- Purge
- GameLeap — ⚠️ checagem de 12/08/2026: Speeed anunciou (jan/2026) que o canal está sendo descontinuado e virando o canal pessoal dele, confirmar
- ZQuixotix
- Dota Alchemy — ⚠️ checagem de 10/08/2026 sugere canal inativo desde 2020, confirmar
- NoobFromUA
- AceDota — https://www.youtube.com/@ace.dota2

## 🎮 Streamers / Pro players
- Gorgc
- SingSing
- AdmiralBulldog
- Topson — https://www.youtube.com/@TopsonOG

## 🗣️ Analistas & Casters
- Aui_2000
- BananaSlamJamma (BSJ) — https://www.youtube.com/@BananaSlamJamma
- RDO Dota
- BalloonDota — nome corrigido em 10/08/2026 (era "BaloonDota")

## 🌍 China
- BurNing
- MickeyToss — ⚠️ checagens de 09/08 e 11/08/2026 não confirmaram conteúdo de Dota 2 recente (resultados se misturam com canais homônimos de outros assuntos), confirmar handle exato

## 🌍 Rússia / CIS
- Kryptonill Gaming
- DotA Digest

## 🏆 Oficiais
- ESL Dota 2
- PGL
- Canal oficial da Valve

## 📈 Outros
- Holy E
- Kardel — ⚠️ checagem de 09/08/2026 não achou um canal claro com esse nome (só resultados sobre o herói Sniper), confirmar
- DATOHLEONG
- NK Dota 2

## 🇧🇷 Brasil
- Astini
- Sensuyy — https://www.youtube.com/@Sensuyy

---

Para adicionar, remover ou reclassificar um canal, edite este arquivo
diretamente. A rotina diária sempre lê a versão mais recente antes de gerar
o briefing.
